...
class PsiformerTest(parameterized.TestCase):
  ...
  def test_antisymmetry(self, jastrow):
    ...
    del jastrow  # not a supported psiformer.make_fermi_net kwarg
  ...
  def test_psiformer(self, **network_options):
    ...
    if network_options['vmap']:
      ...
      network_apply = jax.vmap(network.apply, in_axes=(None, 0, 0))
    ...
    sign_out, log_out = network_apply(params, xs, spins)
