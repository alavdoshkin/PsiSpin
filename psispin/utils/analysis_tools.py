...
def estimate_stats(df: pd.DataFrame,
  ...
  # df.groupby(groups) always yields tuple keys (even for a single grouping
  # column), so pd.DataFrame.from_dict(..., orient='index') below always
  # produces a MultiIndex and reset_index() always names columns level_0,
  # level_1, ... regardless of len(groups).
  ...
  def block(key, values):
    ...
    block_stats = blocking.reblock(values)
    optimal = blocking.find_optimal_block(len(values), block_stats)[0]
    if np.isnan(optimal):
      # Automatic block-length selection failed, most likely because the
      # data is already close to uncorrelated. Fall back to a representative
      # mid-level block rather than the unblocked (block 0) estimate, which
      # underestimates the standard error if any correlation remains.
      optimal = min(3, len(block_stats) - 1)
      logging.warning(
          'Could not find optimal block for %s; using block %d.', key,
          optimal)
    else:
      optimal = int(optimal)
    b = block_stats[optimal]
    return pd.Series({
        'mean': b.mean,
        'standard error': b.std_err,
        'standard error error': b.std_err_err,
    })
  stats = (
