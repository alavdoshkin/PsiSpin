...
import chex
from psispin import checkpoint
from psispin import constants
from psispin import curvature_tags_and_blocks
from psispin import envelopes
from psispin import hamiltonian
from psispin import loss as qmc_loss_functions
from psispin import mcmc
from psispin import networks
from psispin import observables
from psispin import psiformer
from psispin import CustomPsiformer
from psispin import SlaterNet
from psispin.utils import statistics
from psispin.utils import utils
from psispin.utils import writers
from psispin.utils import jax_utils
import jax
...
from psispin.pbc import lattices
...
def init_electrons(
  ...
  """Initializes electron positions around the origin.
  ...
  electron_positions = (
      jax.random.normal(subkey, shape=(batch_size, nelectrons * ndim))
      * init_width
  )
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  Args:
    ...
    writer_manager: context manager with a write method for logging output. If
      None, a default writer (psispin.utils.writers.Writer) is used.
  ...
  if cfg.network.network_type == 'ferminet':
    ...
    network = psiformer.make_fermi_net(
        ...
        complex_output=use_complex,
        pbc_lattice=cfg.system.pbc_lattice,
        use_spin_channels=cfg.network.use_spin_channels,
        **cfg.network.psiformer,
    )
  elif cfg.network.network_type == 'CustomPsiformer':
  ...
  else: # this is applied for ground state computation ( cfg.system.get('states', 0) == 0 )
    logabs_network = lambda *args, **kwargs: signed_network(*args, **kwargs)[1]
  batch_network = jax.vmap(
      logabs_network, in_axes=(None, 0, 0), out_axes=0
  )  # batched network
  ...
  if ckpt_restore_filename:
    ...
    data = networks.FermiNetData(positions=pos, spins=spins)
  ...
  if cfg.mcmc.update_spin:
    train_schema = ['step', 'energy', 'ewmean', 'ewvar', 'pmove', 'pmove_spin', 'locstd']
  else:
  ...
  # Main training
  print("train, make_local_energy_kwargs")
  print(cfg.system.make_local_energy_kwargs)

  # Construct MCMC step
  if cfg.mcmc.update_spin:
    mcmc_step = mcmc.make_mcmc_step_separate_spin_update(
        batch_network,
        device_batch_size,
        steps=cfg.mcmc.steps,
        ndim=cfg.system.ndim,
        blocks=cfg.mcmc.blocks * num_states,
        update_spin=cfg.mcmc.update_spin,
        spin_update_prob=cfg.mcmc.spin_update_prob,
        conserve_sz=cfg.mcmc.conserve_sz,
    )
  else:
  ...
  if cfg.system.make_local_energy_fn:
    ...
    raise ValueError(
        'cfg.system.make_local_energy_fn must be set; there is no default '
        'molecule-based Hamiltonian.')
    ...
    if cfg.system.states > 0 and 'state_scale' not in params:
      state_matrix = utils.select_output(
          networks.make_state_matrix(signed_network,
                                     cfg.system.states), 1)
      batch_state_matrix = jax.vmap(state_matrix, (None, 0, 0))
  ...
  if writer_manager is None:
    if cfg.optim.optimizer == 'none':
      ...
      # for keyword in train_schema[6:]:
      #   stats_name += "_" + keyword
    else:
  ...
  with writer_manager as writer:
    ...
    for t in range(t_init, cfg.optim.iterations):
      ...
      if cfg.mcmc.update_spin: # Extract pmove for spatial moves and pmove for spin moves
        pmove      = pmove[...,0]
        pmove_spin = pmove[...,1]
      ...
      # Update observables
      observable_data = {}
      for keyword, fn in observable_fns.items():
        observable_data[keyword] = fn(params, data, observable_states[keyword])
      ...
      if time.time() - time_of_last_ckpt > cfg.log.save_frequency * 60:
        ...
        if cfg.observables.wavefunction:
          np.save(ckpt_save_path + '/ckpt_' + str(t) + '_sign_logWF.npy', np.array(observable_data['wavefunction']))
        time_of_last_ckpt = time.time()
