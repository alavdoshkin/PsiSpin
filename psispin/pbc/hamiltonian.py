...
- separate Madelung constant computation
...
def make_2DCoulomb_potential(
  ...
  rec = 2 * jnp.pi * jnp.linalg.inv(lattice).T
...
def get_Madelung_constant_2DCoulomb_potential(
  ...
  """ Returns Madelung constant, see Geier PRB B 112, 045119
  ...
  # q = 0 contribution of short-range part, see discussion around eq. (A8) in Geier PRB 112, 045119 (2025)
  phi_S_q0 = (2 * jnp.pi) / area / gamma**0.5 / jnp.pi**0.5
...
def make_Gaussian_potential(
  ...
  """Creates a function to evaluate a short range e-e-interaction potential with two terms:
    U: A Hubbard onsite repulsion of strength: integral dr^2 U(r) = U
  ...
  def real_space_potential(separation: jnp.ndarray):
    """Real-space Ewald potential between charges separated by separation."""
...
def local_energy(
  ...
  if potential_type in {'Coulomb','CoulombMoire'}:
    ...
    potential_energy = make_2DCoulomb_potential(
      lattice, convergence_radius, interaction_energy_scale
    )
  elif potential_type in {'Gaussian','CoulombMoire'}:
    potential_energy = make_Gaussian_potential(
      lattice, convergence_radius,
      potential_kwargs['U'], potential_kwargs['U_width']
    )
  ...
  if potential_type in {'Coulomb'}:
    def _e_l(
      ...
      return 0*potential + kinetic, None


  if potential_type in {'CoulombMoire','GaussianMoire'}:
    ...
    def _e_l(
      ...
      return 0*potential + kinetic + 0*moire_potential, None
