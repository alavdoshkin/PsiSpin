...
def make_loss(network: networks.LogFermiNetLike,
  ...
  """
  # Define function that vmaps local energy computation over the batch of positions
  # with individual keys for individual random numbers
  # Input:
  #   params:
  #   keys: vector of keys of dimension batch_size
  #   data: FermiNetData where positions, spins have dim (batch_size, x)
  ...
  batch_network = jax.vmap(network, in_axes=(None, 0, 0), out_axes=0)
  ...
  def total_energy_jvp(primals, tangents):  # pylint: disable=unused-variable
    ...
    primals = (primals[0], data.positions, data.spins)
    tangents = (
        tangents[0],
...
def make_wqmc_loss(
  ...
  batch_network = vmap(network, in_axes=(None, 0, 0), out_axes=0)
  ...
  def total_energy_jvp(primals, tangents):  # pylint: disable=unused-variable
    ...
    def log_q(params_, pos_, spins_):
      out = batch_network(params_, pos_, spins_)
      kfac_jax.register_normal_predictive_distribution(out[:, None])
      return out.sum()

    score = jax.grad(log_q, argnums=1)
    primals = (params, data.positions, data.spins)
    tangents = (
        tangents[0],
